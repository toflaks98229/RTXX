...
public partial class Army : MonoBehaviour
{
    ...
    /// <summary>
    /// 4단계: Job 결과를 유닛에 반영하고 사후 정산을 합니다.
    ///
    /// ---------------------------------------------------------------------
    /// 시도했다가 되돌린 최적화 (같은 길을 다시 가지 않도록 남깁니다)
    /// ---------------------------------------------------------------------
    /// 아래 U_Fight 루프(유닛마다 u._Update())가 2.2ms로 이 단계의 절반을
    /// 차지합니다. 하는 일은 대부분 unit_Data 안에서 끝나는 값 연산이라
    /// Burst Job으로 옮기는 것이 자연스러워 보입니다. 세 번 시도했습니다.
    ///
    ///   1) 부대별 Job + 즉시 Complete()
    ///      U_Fight 2.22 -> 0.12 ms  (일감은 확실히 줄어듦)
    ///      Apply   4.66 -> 7.22 ms  (그러나 전체는 악화)
    ///
    ///   2) 전 부대 Job을 걸어 두고 마지막에 한 번만 Complete()
    ///      Apply   7.26 ms  (거의 그대로. Complete 왕복이 원인이 아니었음)
    ///
    ///   3) 슬롯 Transform을 '필요한 유닛만' 읽도록 축소
    ///      Apply   5.66 ms  (개선했으나 여전히 기준선보다 나쁨)
    ///
    /// 원인: Job에 넘길 입력(진형 슬롯 위치)을 메인 스레드가 모아야 하는데,
    /// 그 수집이 Transform 읽기라 Job으로 아낀 것을 도로 까먹습니다.
    /// 슬롯은 Transform으로 존재하므로 이 수집을 피할 방법이 현 구조에는
    /// 없습니다. (진형 슬롯을 Transform이 아닌 순수 데이터로 바꾸면
    /// 성립하지만, 그건 진형 시스템 전체를 건드리는 별도 작업입니다)
    ///
    /// 기준선 15.10 ms가 세 시도(17.28 / 18.46 / 18.95)보다 모두 낫습니다.
    /// </summary>
