...
public partial struct Unit_Data
{
    ...
    /// <summary>
    /// 이 유닛을 쓰러뜨린 유닛이 속한 부대의 인덱스입니다. 없으면 -1입니다.
    ///
    /// 왜 필요한가:
    /// 예전에는 killerNum만 기록하고, 킬을 준 부대를 찾기 위해 사망할 때마다
    /// 타겟 부대와 탐지된 부대들의 '전 유닛'을 순회하며 번호를 비교했습니다.
    /// (Army.Find_Army_Of_Unit) 사망 1건당 O(부대수 x 인원)이라
    /// 대량 사상자가 나는 순간 비용이 몰렸습니다.
    /// 공격 시점에 이미 알고 있는 소속 인덱스를 함께 남기면 O(1)로 끝납니다.
    /// </summary>
    public int killerArmyIndex;
