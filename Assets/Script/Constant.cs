...
static class Constant
{
    ...
    // 공격 모션 (Attack Lunge)
    //
    // 토탈워는 두 병사를 짝지어 전용 애니메이션을 재생합니다(매치드 컴뱃).
    // 스프라이트 기반인 이 프로젝트에서 그걸 흉내 내려면 비용이 큰 데 비해
    // 전술적 이득은 없습니다. 대신 '표적 쪽으로 내지르는' 짧은 전진 모션으로
    // 누가 누구를 치고 있는지 눈에 보이게 합니다.
    // ---------------------------------------------------------------------
    /// <summary>공격 시 표적 쪽으로 내지르는 거리입니다.</summary>
    public const float attack_Lunge_Distance = 0.35f;

    /// <summary>내지르기가 끝나고 원위치로 돌아오기까지의 시간(초)입니다.</summary>
    public const float attack_Lunge_Time = 0.18f;
