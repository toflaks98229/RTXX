...
**1~3단계는 채택, 4단계는 반증되어 되돌렸습니다.**

측정 조건: 9,600명, 700틱, seed 12345, `-profile`

| 단계 | 구성 | Apply | 합계 |
|---|---|---|---|
| 기준선 | Transform 슬롯 + 메인스레드 | 4.03 | 13.18 |
| 1단계 | 배열 병행 (읽기는 Transform) | — | — |
| 2단계 | 배열 읽기 + 대조 검증 병행 | 4.19 | 13.66 |
| **3단계** | **Transform 슬롯 제거** | **2.59** | **11.75** |
| 4단계a | 배열 + Job (즉시 Complete) | 3.51 | 12.60 |
| 4단계b | 배열 + Job (지연 Complete) | 3.25 | 12.08 |
| **최종** | **3단계 구성으로 되돌림** | **2.75** | **11.62** |

### 얻은 것

- **Apply 4.03 → 2.75 ms** (약 32% 감소)
- **합계 13.18 → 11.62 ms** (약 12% 감소)
- **GameObject 9,600개 제거** (유닛당 빈 슬롯 오브젝트)
- 슬롯 대조 불일치 **0건** (1·2단계 검증 프로브)
- 예외 0건, 사망 550(기준선 547~551 범위), 돌격 20회

### 4단계가 반증된 이유 — 이 문서의 핵심 오류

이 문서는 *"과거 Job 전환이 실패한 원인은 입력 수집이므로, 슬롯을 배열로
바꾸면 Job이 비로소 이득이 된다"* 고 예측했습니다. **틀렸습니다.**

입력 수집을 완전히 없앤 뒤에도 Job이 메인 스레드보다 **느렸습니다**(3.25 vs 2.59).
지연 Complete로 스케줄 왕복을 줄여도 0.26 ms만 회복될 뿐이었습니다.

진짜 원인은 **유닛당 작업량**입니다. 이 후처리는 벡터 몇 번 더하고 분기 두 개를
타는 것이 전부라, 워커에 일감을 나눠 주고 거둬들이는 비용이 계산 자체보다 큽니다.
반면 `unit_Datas`는 이미 조밀한 배열이라 메인 스레드 순회도 충분히 캐시 친화적입니다.

즉 **이득의 원천은 Job이 아니라 Transform 제거였습니다.** 아래 3절의 예측
(`12ms대 진입`)은 결과적으로 맞았지만, 그 경로는 4단계가 아니라 3단계였습니다.

`Unit_Post_Move_Job` 구조체는 `Unit_Job.cs`에 남겨 두었습니다. 유닛당 작업량이
크게 늘어나면(유닛별 경로탐색, 개별 회피 조향 등) 판단이 뒤집힐 수 있으므로,
그때 다시 측정하라는 표식입니다.

### 검증에 관한 주의

`stateHash`는 **판정 기준으로 쓸 수 없습니다.** 같은 코드를 같은 시드로 두 번
돌려도 값이 다릅니다(사망 547 vs 551, 0.7% 편차). 이는 `Simulation_Clock.cs`에
기록된 잔여 비결정론(밀어내기 누적 순서)이 여전히 살아 있기 때문입니다.

따라서 판정은 **편차 범위**로 해야 합니다. 이번 작업의 모든 단계는 사망자 수가
546~560 범위에 들어 기준선 편차(3.1%) 안이었습니다.
